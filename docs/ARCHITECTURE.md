...
- `LiteLocalModelRuntime` and the native GGUF tokenizer proof bridge surface `tokenizer_loaded`, `vocab_size`, `tokenization_ok`, and `token_ids_preview` only when the GGUF metadata and vocabulary mapping were actually parsed and verified; `real_inference` and `native_generation_available` stay false, `decoded_text` stays null, and the bridge returns `REAL_NATIVE_INFERENCE_NOT_IMPLEMENTED` until actual tensor/logit computation exists
- The debug-only diagnostic broadcast receiver now bypasses `LocalMockBrainProvider`, prefers the `command` extra, and drives the native GGUF probe directly so phone logs can show the exact model path, enablement state, and tokenizer proof status
